...
export default function EmotionalBalanceBar({
  ...
  const { textMain, textSub, divider } = useThemeVars();
  ...
  return (
    <View style={{ width }}>
      ...
      <View style={[styles.barZone, { height: maxBarHeight + 14 }]}>
        {/* Left bar (positive) */}
        <View style={[styles.barCol, { alignItems: 'flex-end' }]}>
          <View style={{ height: maxBarHeight, justifyContent: 'flex-end' }}>
            <View
              style={[
                styles.bar,
                { height: posH, backgroundColor: positiveColor },
              ]}
            />
          </View>
        </View>
        ...
        <View
          style={[
            styles.centerBox,
            { width: gapPx, minWidth: gapPx, maxWidth: gapPx },
          ]}
        >
          <Text style={{ color: textMain, fontSize: 12, fontWeight: '800' }}>
            {ratioText}
          </Text>
        </View>
        ...
        <View style={[styles.barCol, { alignItems: 'flex-start' }]}>
          <View style={{ height: maxBarHeight, justifyContent: 'flex-end' }}>
            <View
              style={[
                styles.bar,
                { height: negH, backgroundColor: negativeColor },
              ]}
            />
          </View>
        </View>
      ...
      {/* Optional legend under bars */}
      {showLegend && (
        <View style={styles.legendRow}>
          <LegendDot color={positiveColor} />
          <Text style={[styles.legendText, { color: textSub }]}>Calm / Positive</Text>
          <View style={{ width: 14 }} />
          <LegendDot color={negativeColor} />
          <Text style={[styles.legendText, { color: textSub }]}>Tension / Difficult</Text>
        </View>
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  bar: {
    width: 16,       // bar thickness (adjust if needed)
