 ...
 *  - First tap -> sets {from = date, to = null}
 *  - Second tap:
 *      * if second < first  -> cancel previous and set {from = second, to = null}
 *      * if second >= first -> set {from = first, to = second} (inclusive range)
 *  - Third tap -> start a new selection from tapped date: {from = date, to = null}
 *
 * Visuals:
 *  - Booking-like trail for [from..to] (rounded ends)
 *  - Emotion dot per day (first color from getEmotionMeta)
 *  - All colors adapt to theme
 */
export default function HistoryCalendar({ history, value, onChange }) {
  const t = useThemeVars();
  const { cardBg, textMain, button } = t;

    // Control visible month explicitly so Calendar doesn't jump back to today
    const [currentMonth, setCurrentMonth] = useState(() => {
    // Prefer the 'from' date as initial month; fallback to today
    const seed = value?.from || new Date().toISOString().slice(0, 10);
    return seed;
    });

  // Accent for the selection "trail"
  const selectionColor = button; // consistent with your buttons
  ...
  // Compute marking object using "custom" + custom dayComponent to fully control visuals
  const range = normalizeRange(value);
  const isInRange = useCallback((key) => {
    if (!range.from) return false;
    if (!range.to) return key === range.from;
    return key >= range.from && key <= range.to;
  }, [range.from, range.to]);

  // NOTE: We will use dayComponent to draw dots and the booking-like background.
  // Calendar theme uses cardBg/textMain; we keep calendar chrome minimal.
  const calendarTheme = {
    calendarBackground: '#FFFFFF',
    monthTextColor: textMain,
    textSectionTitleColor: rgba(textMain, 0.8),
    dayTextColor: textMain,
    todayTextColor: textMain,
    arrowColor: textMain,
    textDisabledColor: rgba(textMain, 0.4),
  };
  ...
  const onDayPress = (day) => {
   ...
   onChange({ from: key, to: key });
    ...
    return (
       ...
       // Optional: pressing a day from another month will NOT auto-switch month
       disableMonthChange={true}
        // We use a custom day component to combine range highlight + emotion dots
        dayComponent={({ date, state }) => {
            const key = date?.dateString;
            const selected = key && isInRange(key);
            const isSingle = key && range.from && key === range.from && !range.to;
          ...
          >
            <View style={[styles.tile, isDisabled && { opacity: 0.5 }]}>
              ...
              <View
                style={[
                  styles.selection,
                  selected && { backgroundColor: rgba(selectionColor, 0.55) },
                ]}
              />
        ...
        // We do not use built-in markingType to keep full control via dayComponent
      />
    </View>
  );
}
...
function rgba(hex, alpha) {
  // Accept #RRGGBB or #RGB. Fallback to accent if parsing fails.
  try {
    const h = hex.replace('#', '');
